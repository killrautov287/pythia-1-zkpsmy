...
describe("Pythia 1 Verifier contract", () => { 
  ...
  before(async () => {
      ...
      pythia1VerifierContract = Pythia1Verifier__factory.connect(deployed.address, deployer);
  });
  ...
  it("Should be able to verify the proof using the verifier", async () => {
    ...
    const isValidContract = await pythia1VerifierContract.verifyProof(proofBytes, snarkProof.input);
